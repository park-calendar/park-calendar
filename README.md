...
| `data/config.json` | 표시 월 범위, 카테고리, 월별 안내 문구, 최종 갱신일 |
| `data/YYYY-MM.json` | 해당 월에 시작하는 행사 목록. **루틴이 갱신하는 파일** |
| `scripts/merge_events.py` | 새 행사 JSON 을 월 파일에 병합(중복 제거·갱신) |
| `scripts/new_events.sample.json` | 루틴이 만들어야 하는 입력 형식 예시 |
| `scripts/deploy_ncp.sh` | NCP Object Storage 업로드 (`NCP_BUCKET=버킷 ./scripts/deploy_ncp.sh`) |
...
2. `python3 scripts/merge_events.py new_events.json`
...
3. 업로드: `NCP_BUCKET=<버킷> ./scripts/deploy_ncp.sh`
4. 데이터만 갱신할 때: `NCP_BUCKET=<버킷> ./scripts/deploy_ncp.sh data`
